...
SIZE BNImageView::expectedRenderSize(BNImage* image, SIZE containingSize) {
	SIZE size = containingSize;
	size.height = containingSize.width * (image->_height/(float)image->_width);
	return size;
}
...
void BNImageView::setBNImage(BNImage* image) {
	float width = _rect.size.width * app->_defaultDisplay->_scale;
