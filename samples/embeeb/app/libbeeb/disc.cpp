...
struct _loader {
        char const *ext;
        void (*load)(int drive, unsigned char *img, int cb);
...
void loaddisc(int drive, int loader, unsigned char* discimg, int cb) {
	driveloaders[drive]=loader;
	loaders[loader].load(drive, discimg, cb);
...
void disc_seek(int drive, int track)
{
        ...
        fdctime=200;//ddnoise_seek(track-oldtrack[drive]);
...
uint8_t* serialize_disc(bool saving, uint8_t* p) {
	...
	SERIALIZE(disctime);
	SERIALIZE(motorspin);
	SERIALIZE(driveloaders);
	SERIALIZE(disc_notfound);
	SERIALIZE(oldtrack);
	SERIALIZE(curdrive);
	return p;
}
