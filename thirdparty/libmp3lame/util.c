                     ...
                     int desired_len, sample_t const *inbuf, int len, int *num_used, int ch)
{
    ...
    if (gfc->fill_buffer_resample_init == 0) {
        esv->inbuf_old[0] = (sample_t*)calloc(BLACKSIZE, sizeof(esv->inbuf_old[0][0]));
        esv->inbuf_old[1] = (sample_t*)calloc(BLACKSIZE, sizeof(esv->inbuf_old[0][0]));
        for (i = 0; i <= 2 * bpc; ++i)
            esv->blackfilt[i] = (sample_t*)calloc(BLACKSIZE, sizeof(esv->blackfilt[0][0]));
