...
void reorder_stages(struct PMultiStageDef* pdef) {
	int* ratios = (int*) malloc(2 * pdef->nb_stages * sizeof(int));
...
void check_stages(struct PMultiStageDef* pdef) {
	// sort ratios
	int* at_end = (int*)malloc(2* pdef->nb_stages * sizeof(int));
	int end_index = 0;
	int pos_index = 0;
	int Mprod = 1;
	int Lprod = 1;
...
struct PMultiStageDef* get_predef_ratios(int fsin, int fsout) {
	...
	for (int i = 0; i < s_predefs_size; i++) {
		if ((s_predefs[i].P == P) && (s_predefs[i].Q == Q)) {
			struct PMultiStageDef* pdef = (PMultiStageDef*)malloc(sizeof(struct PMultiStageDef));
			const char* def = s_predefs[i].def;
			pdef->nb_stages = 0;
			for (int c=0;c<strlen(def);c++)
				if (def[c]=='/') pdef->nb_stages++;
			pdef->L = (int*)malloc(2 * pdef->nb_stages * sizeof(int));
...
struct PMultiStageDef* build_auto_ratios(int fsin, int fsout, double tol) {
	...
	int nbL = MAX_FACTORS;
	int* LL = (int*)malloc(nbL * sizeof(int));
	...
	int nbM = MAX_FACTORS;
	int* MM = (int*)malloc(nbM * sizeof(int));
	...
	struct PMultiStageDef* pdef = (PMultiStageDef*)malloc(sizeof(struct PMultiStageDef));
	pdef->nb_stages = nbL;
	pdef->L = (int*)malloc(2 * nbL * sizeof(int));
	pdef->M = &pdef->L[nbL];
	for (int s = 0; s < nbL; s++) {
		pdef->L[s] = LL[s];
		pdef->M[s] = MM[s];
...
//	printf("build fast ratios for %i/%i\n",L,M);
	...
	struct PMultiStageDef* best = (PMultiStageDef*)malloc(2*sizeof(struct PMultiStageDef) + 4*MAX_NB_STAGES*sizeof(int));
	best->L = (int*) (best + 1);
...
//	printf("width %0.2f operations\n",best_ops);
	...
	pdef->L = (int*)malloc(2 * best->nb_stages * sizeof(int));
