		...
		double** FreqGrille, int* taillegrilledense, double** des, double** wt)
{
	...
	*FreqGrille = (double*)malloc(*taillegrilledense * sizeof(double));
	*des =(double*) malloc(*taillegrilledense * sizeof(double));
	*wt = (double*)malloc(*taillegrilledense * sizeof(double));
...
void build_rep_imp(int nfcns, const double* ad, double* x, const double* y, int nodd, double* h)
{
	...
	double* a = (double*)malloc((nfcns+1)*sizeof(double));
	double* alpha = (double*)malloc((nfcns+2)*sizeof(double));
	int nz = nfcns+1;
	int nzz = nz+1;
	int nm1 = nfcns-1;
	double fsh = 1.0e-6;
           ...
           double bands[], double dev[], double weight[])
{
	...
	// Extrema initiaux
	int* iExt = (int*)malloc((nfcns+2)*sizeof(int));
	{
	...
	// Algorithme d'echange
	double* x = (double*)malloc((nfcns+2)*sizeof(double));
	double* y = (double*)malloc((nfcns+1)*sizeof(double));
	double* ad = (double*)malloc((nfcns+1)*sizeof(double));
