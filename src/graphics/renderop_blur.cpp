...
void BlurRenderOp::validateShader(RenderTask* r) {
    if (!_alloc) {
        _alloc = r->_renderer->allocQuads(1, NULL);
    }
    
    
    _blurShader = new BlurShader(r->_renderer);
    _shader = new PostBlurShader(r->_renderer);

    _tex1 = r->_renderer->createTexture(r->_renderer->_primarySurfaceFormat);
    _tex1->_maxMipMapLevel = 2;
    _tex1->_minFilterLinear = true;
    _tex1->_mipFilterLinear = false;
    _dirty = true;
    
    _surface1 = r->_renderer->createSurface(true);
    _surface2 = r->_renderer->createSurface(true);
...
void BlurRenderOp::prepareToRender(RenderTask* r, Surface* surface) {
    ...
    r->setCurrentTexture(_tex1);
    r->copyFromCurrent(rect, _tex1, {0,0});
    
    // Generate 2 levels of mipmap to scale down by 1/4
    r->generateMipmaps(_tex1);

    // Prepare to render to offscreen surface
    r->setCurrentShader(_blurShader);
    r->setBlendNone();
    r->setUniform(_blurShader->_u_mvp, _mvp);
    ...
    r->setCurrentSurface(_surface1);
    r->setUniform(blurShader->_u_texOffset, VECTOR2(0, 1.f/_downsampledSize.height));
    r->draw(Quad, 1, _alloc->offset);
    
    // 1D blur vertically, source is surface1 and destination is surface2
    r->setCurrentSurface(_surface2);
    r->setCurrentTexture(_surface1->_texture);
    r->setUniform(blurShader->_u_texOffset, VECTOR2(1.f/_downsampledSize.width,0));
    r->draw(Quad, 1, _alloc->offset);
    
    r->setCurrentSurface(surface);
    r->setCurrentTexture(_surface2->_texture);
