self.onmessage = function(e) {
    var width = e.data.width;
    var height = e.data.height;
    var data = e.data.data;
    var roi = {};
    roi.left = Math.floor(e.data.roiLeft);
    roi.top = Math.floor(e.data.roiTop);
    roi.width = Math.ceil(e.data.roiWidth);
    roi.height = Math.ceil(e.data.roiHeight);
    var nf = face_detector.detect(data, width, height, roi, 0.05);
    var result = nf ? nf.length : 0;
    if (result > 0) {
        console.log("face!");
    }
    postMessage(result);
};
(function() {
 face_detector = {};
 
 var stageData = [
                  0.822689, 3,
                  6.956609, 16,
                  9.498543, 21,
                  18.412970, 39,
                  15.324140, 33,
                  21.010639, 44,
                  23.918791, 50,
                  24.527880, 51,
                  27.153351, 56,
                  34.554111, 71,
                  39.107288, 80,
                  50.610481, 103,
                  54.620071, 111,
                  50.169731, 102,
                  66.669121, 135,
                  67.698921, 137,
                  69.229874, 140,
                  79.249077, 160,
                  87.696030, 177,
                  90.253349, 182,
                  104.749199, 211,
                  105.761101, 213,
                  ];
 
 
 var nodeData = "";
 
 
 var rectData = "";
 
 
 // Decode the node data into a float32 array and a separate nodeNumRects array
 var binary_string = atob(nodeData);
 var len = binary_string.length;
 var bytes = new Uint8Array( len );
 for (var i = 0; i < len; i++)        {
 bytes[i] = binary_string.charCodeAt(i);
 }
 var decodedNodeData = new Float32Array(bytes.buffer);
 var nodesNumRectsData = new Uint8Array(decodedNodeData.length);
 var n=0;
 for (var i=0 ; i<decodedNodeData.length ; i+=3) {
 var numRects = 1;
 var nodeLeft =  decodedNodeData[i+1];
 var nodeRight =  decodedNodeData[i+2];
 if (nodeLeft < 0 || (nodeLeft===0 && ((1/nodeLeft)<0))) { // detect -0
 numRects=2;
 decodedNodeData[i+1] = -nodeLeft;
 }
 if (nodeRight < 0 || (nodeRight===0 && ((1/nodeRight)<0))) {
 numRects=3;
 decodedNodeData[i+2] = -nodeRight;
 }
 nodesNumRectsData[n++] = numRects;
 }
 
 
 // Decode rect data. Each encoded rect is 20 bits, they decode to 40 bits (5 bytes)
 binary_string = atob(rectData);
 len = binary_string.length;
 var decodedRectData = new Int8Array(len * 2);
 var o=0;
 var oddrect = false;
 var prev = 0|0;
 for (var i = 0; i < len; ) {
 var rectCoded=0|0;
 if (!oddrect) {
 rectCoded = binary_string.charCodeAt(i++) |0;
 rectCoded |= (binary_string.charCodeAt(i++)<<8);
 prev = binary_string.charCodeAt(i++) |0;
 rectCoded |= ((prev>>4)<<16) |0;
 prev = (prev & 0xf)|0;
 } else {
 rectCoded = prev|0;
 rectCoded |= (binary_string.charCodeAt(i++)<<4);
 rectCoded |= (binary_string.charCodeAt(i++)<<12);
 }
 oddrect = !oddrect;
 
 // Decoding
 var rectWeight = rectCoded&3;
 if (rectWeight==0) rectWeight=-1;
 rectCoded>>=2;
 var rectHeight = 1 + rectCoded % 20;
 rectCoded = Math.floor(rectCoded / 20);
 var rectWidth = 1 + rectCoded % 20;
 rectCoded = Math.floor(rectCoded / 20);
 var rectTop = rectCoded % 20;
 rectCoded = Math.floor(rectCoded / 20);
 var rectLeft = rectCoded % 20;
 
 // Write out the five rect properties as bytes
 decodedRectData[o++] = rectLeft;
 decodedRectData[o++] = rectTop;
 decodedRectData[o++] = rectWidth;
 decodedRectData[o++] = rectHeight;
 decodedRectData[o++] = rectWeight;
 }
 
 
 
 
 
 /**
  * Holds the minimum area of intersection that defines when a rectangle is
  * from the same group. Often when a face is matched multiple rectangles are
  * classified as possible rectangles to represent the face, when they
  * intersects they are grouped as one face.
  * @type {number}
  * @default 0.5
  * @static
  */
 face_detector.REGIONS_OVERLAP = 0.5;
 
 /**
  * Detects through the HAAR cascade data rectangles matches.
  * @param {pixels} pixels The pixels in a linear [r,g,b,a,...] array.
  * @param {number} width The image width.
  * @param {number} height The image height.
  * @param {number} initialScale The initial scale to start the block
  *     scaling.
  * @param {number} scaleFactor The scale factor to scale the feature block.
  * @param {number} stepSize The block step size.
  * @param {number} edgesDensity Percentage density edges inside the
  *     classifier block. Value from [0.0, 1.0], defaults to 0.2. If specified
  *     edge detection will be applied to the image to prune dead areas of the
  *     image, this can improve significantly performance.
  *
  *   edgesDensity: 0.05
  *
  * @return {array} Found rectangles.
  * @static
  */
 face_detector.detect = function(pixels, width, height, roi, edgesDensity) {
 var total = 0;
 var rects = [];
 
 // Convert ROI region of image to a new greyscale image
 var gray = new Uint8Array(roi.width * roi.height);
 var pixels32 = new Uint32Array(pixels.buffer, pixels.byteOffset, pixels.length>>2);
 var i=0;
 for (var y=roi.top; y<(roi.top+roi.height) ; y++) {
 var yy = (y*width)|0;
 for (var x=roi.left; x<(roi.left+roi.width); x++) {
 var c = pixels32[yy+x];
 var luma = ((c>>>16&0xff) * 13933 + (c>>>8&0xff) * 46871 + (c&0xff) * 4732)>>>16;
 gray[i++] = luma;
 }
 }
 width = roi.width;
 height = roi.height;
 
 // Create Sobel-filtered image from the grayscale image
 var pixelsSobel = new Float32Array(width * height);
 var sobelSignVector = new Float32Array([-1, 0, 1]);
 var sobelScaleVector = new Float32Array([1, 2, 1]);
 var vertical = this.separableConvolve(gray, width, height, sobelSignVector, sobelScaleVector);
 var horizontal = this.separableConvolve(gray, width, height, sobelScaleVector, sobelSignVector);
 for (var i = 0; i < pixelsSobel.length; i++) {
 var v = vertical[i];
 var h = horizontal[i];
 var p = Math.sqrt(h * h + v * v);
 pixelsSobel[i] = p;
 }
 
 
 // Create the integral images
 var integralImage = new Int32Array(width * height);
 var integralImageSquare = new Int32Array(width * height);
 var integralImageSobel = new Int32Array(width * height);
 for (var i = 0; i < height; i++) {
 for (var j = 0; j < width; j++) {
 var w = i * width + j;
 var pixel = gray[w];
 integralImage[w] =  (integralImage[w - width] || 0)
 + (integralImage[w - 1] || 0)
 + pixel
 - (integralImage[w - width - 1] || 0);
 
 integralImageSquare[w] =  (integralImageSquare[w - width] || 0)
 + (integralImageSquare[w - 1] || 0)
 + pixel*pixel
 - (integralImageSquare[w - width - 1] || 0);
 
 integralImageSobel[w] =  (integralImageSobel[w - width] || 0)
 + (integralImageSobel[w - 1] || 0)
 + pixelsSobel[w]
 - (integralImageSobel[w - width - 1] || 0);
 }
 }
 
 
 var minWidth = 20;
 var minHeight = 20;
 
 // Iterate from 50% of ROI width up to 100% in increments of 10%
 for (var scale=0.5; scale<=1.0; scale+=0.1) {
 var blockScale = (scale * width) / minWidth;
 var blockWidth = (blockScale * minWidth) | 0;
 var blockHeight = (blockScale * minHeight) | 0;
 
 var step = (blockWidth/8) | 0;
 
 // Scan the image a block at a time top->bottom, left->right. Blocks overlap a lot.
 for (var i = 0; i < (height - blockHeight); i += step) {
 for (var j = 0; j < (width - blockWidth); j += step) {
 
 // Quick sobel edge test. If there's not enough edges in the block, discard it
 if (edgesDensity > 0) {
 var wbA = i * width + j;
 var wbB = wbA + blockWidth;
 var wbD = wbA + blockHeight * width;
 var wbC = wbD + blockWidth;
 var blockEdgesDensity = (integralImageSobel[wbA] - integralImageSobel[wbB] - integralImageSobel[wbD] + integralImageSobel[wbC]) / (blockWidth * blockHeight * 255);
 if (blockEdgesDensity < edgesDensity) {
 continue;
 }
 }
 
 // Test block for presence of a face
 if (this.evalStages_(integralImage, integralImageSquare, i, j, width, blockWidth, blockHeight, blockScale)) {
 rects[total++] = {
 width: blockWidth,
 height: blockHeight,
 x: j,
 y: i
 };
 }
 }
 }
 }
 //console.log("skipped=" + numSkipped + " tested=" + numTested);
 
 return this.mergeRectangles_(rects);
 };
 
 
 
 /**
  * Evaluates if the block size on i,j position is a valid HAAR cascade
  * stage.
  * @param {number} i Vertical position of the pixel to be evaluated.
  * @param {number} j Horizontal position of the pixel to be evaluated.
  * @param {number} width The image width.
  * @param {number} blockSize The block size.
  * @param {number} scale The scale factor of the block size and its original
  *     size.
  * @param {number} inverseArea The inverse area of the block size.
  * @return {boolean} Whether the region passes all the stage tests.
  * @private
  * @static
  */
 face_detector.evalStages_ = function(integralImage, integralImageSquare, i, j, width, blockWidth, blockHeight, scale) {
 var inverseArea = 1.0 / (blockWidth * blockHeight);
 var wbA = i * width + j;
 var wbB = wbA + blockWidth;
 var wbD = wbA + blockHeight * width;
 var wbC = wbD + blockWidth;
 var mean = (integralImage[wbA] - integralImage[wbB] - integralImage[wbD] + integralImage[wbC]) * inverseArea;
 var variance = (integralImageSquare[wbA] - integralImageSquare[wbB] - integralImageSquare[wbD] + integralImageSquare[wbC]) * inverseArea - mean * mean;
 
 var standardDeviation = 1;
 if (variance > 0) {
 standardDeviation = Math.sqrt(variance);
 }
 
 var n=0;
 var ri=0;
 var nr=0;
 for (var s=0 ; s<stageData.length ; s+=2) {
 var stageSum = 0;
 var stageThreshold = stageData[s];
 var nodeLength = stageData[s+1];
 
 while (nodeLength--) {
 var nodeThreshold = decodedNodeData[n++];
 var nodeLeft = decodedNodeData[n++];
 var nodeRight = decodedNodeData[n++];
 var rectsLength = nodesNumRectsData[nr++];
 
 var rectsSum = 0;
 
 for (var r = 0; r < rectsLength; r++) {
 var rectLeft = (j + decodedRectData[ri++] * scale + 0.5) | 0;
 var rectTop = (i + decodedRectData[ri++] * scale + 0.5) | 0;
 var rectWidth = (decodedRectData[ri++] * scale + 0.5) | 0;
 var rectHeight = (decodedRectData[ri++] * scale + 0.5) | 0;
 var rectWeight = decodedRectData[ri++];
 
 // RectSum(r) = SAT(x-1, y-1) + SAT(x+w-1, y+h-1) - SAT(x-1, y+h-1) - SAT(x+w-1, y-1)
 var  w1 = rectTop * width + rectLeft;
 var  w2 = w1 + rectWidth;
 var  w3 = w1 + rectHeight * width;
 var  w4 = w3 + rectWidth;
 rectsSum += (integralImage[w1] - integralImage[w2] - integralImage[w3] + integralImage[w4]) * rectWeight;
 }
 
 if (rectsSum * inverseArea < nodeThreshold * standardDeviation) {
 stageSum += nodeLeft;
 } else {
 stageSum += nodeRight;
 }
 }
 
 if (stageSum < stageThreshold) {
 return false;
 }
 }
 return true;
 };
 
 face_detector.intersectRect = function(x0, y0, x1, y1, x2, y2, x3, y3) {
 return !(x2 > x1 || x3 < x0 || y2 > y1 || y3 < y0);
 };
 
 
 face_detector.DisjointSet = function(length) {
 if (length === undefined) {
 throw new Error('DisjointSet length not specified.');
 }
 this.length = length;
 this.parent = new Uint32Array(length);
 for (var i = 0; i < length; i++) {
 this.parent[i] = i;
 }
 };
 face_detector.DisjointSet.prototype.length = null;
 face_detector.DisjointSet.prototype.parent = null;
 face_detector.DisjointSet.prototype.find = function(i) {
 if (this.parent[i] === i) {
 return i;
 } else {
 return (this.parent[i] = this.find(this.parent[i]));
 }
 };
 face_detector.DisjointSet.prototype.union = function(i, j) {
 var iRepresentative = this.find(i);
 var jRepresentative = this.find(j);
 this.parent[iRepresentative] = jRepresentative;
 };
 
 
 face_detector.mergeRectangles_ = function(rects) {
 var disjointSet = new this.DisjointSet(rects.length);
 
 for (var i = 0; i < rects.length; i++) {
 var r1 = rects[i];
 for (var j = 0; j < rects.length; j++) {
 var r2 = rects[j];
 if (this.intersectRect(r1.x, r1.y, r1.x + r1.width, r1.y + r1.height, r2.x, r2.y, r2.x + r2.width, r2.y + r2.height)) {
 var x1 = Math.max(r1.x, r2.x);
 var y1 = Math.max(r1.y, r2.y);
 var x2 = Math.min(r1.x + r1.width, r2.x + r2.width);
 var y2 = Math.min(r1.y + r1.height, r2.y + r2.height);
 var overlap = (x1 - x2) * (y1 - y2);
 var area1 = (r1.width * r1.height);
 var area2 = (r2.width * r2.height);
 
 if ((overlap / (area1 * (area1 / area2)) >= this.REGIONS_OVERLAP) &&
     (overlap / (area2 * (area1 / area2)) >= this.REGIONS_OVERLAP)) {
 disjointSet.union(i, j);
 }
 }
 }
 }
 
 var map = {};
 for (var k = 0; k < disjointSet.length; k++) {
 var rep = disjointSet.find(k);
 if (!map[rep]) {
 map[rep] = {
 total: 1,
 width: rects[k].width,
 height: rects[k].height,
 x: rects[k].x,
 y: rects[k].y
 };
 continue;
 }
 map[rep].total++;
 map[rep].width += rects[k].width;
 map[rep].height += rects[k].height;
 map[rep].x += rects[k].x;
 map[rep].y += rects[k].y;
 }
 
 var result = [];
 Object.keys(map).forEach(function(key) {
                          var rect = map[key];
                          result.push({
                                      total: rect.total,
                                      width: (rect.width / rect.total + 0.5) | 0,
                                      height: (rect.height / rect.total + 0.5) | 0,
                                      x: (rect.x / rect.total + 0.5) | 0,
                                      y: (rect.y / rect.total + 0.5) | 0
                                      });
                          });
 
 return result;
 };
 
 
 
 
 face_detector.separableConvolve = function(pixels, width, height, horizWeights, vertWeights) {
 var vertical = this.verticalConvolve(pixels, width, height, vertWeights);
 return this.horizontalConvolve(vertical, width, height, horizWeights);
 };
 face_detector.horizontalConvolve = function(pixels, width, height, weightsVector) {
 var side = weightsVector.length;
 var halfSide = Math.floor(side / 2);
 var output = new Float32Array(width * height);
 
 for (var y = 0; y < height; y++) {
 for (var x = 0; x < width; x++) {
 var sy = y;
 var sx = x;
 var offset = (y * width + x);
 var r = 0;
 for (var cx = 0; cx < side; cx++) {
 var scy = sy;
 var scx = Math.min(width - 1, Math.max(0, sx + cx - halfSide));
 var poffset = (scy * width + scx);
 var wt = weightsVector[cx];
 r += pixels[poffset] * wt;
 }
 output[offset] = r;
 }
 }
 return output;
 };
 face_detector.verticalConvolve = function(pixels, width, height, weightsVector) {
 var side = weightsVector.length;
 var halfSide = Math.floor(side / 2);
 var output = new Float32Array(width * height);
 
 for (var y = 0; y < height; y++) {
 for (var x = 0; x < width; x++) {
 var sy = y;
 var sx = x;
 var offset = (y * width + x);
 var r = 0;
 for (var cy = 0; cy < side; cy++) {
 var scy = Math.min(height - 1, Math.max(0, sy + cy - halfSide));
 var scx = sx;
 var poffset = (scy * width + scx);
 var wt = weightsVector[cy];
 r += pixels[poffset] * wt;
 }
 output[offset] = r;
 }
 }
 return output;
 };
 
 
 }());

