...
static int jo_processDU(ByteBufferStream& out, int &bitBuf, int &bitCnt, float *CDU, float *fdtbl, int DC, const unsigned short HTDC[256][2], const unsigned short HTAC[256][2]) {
    ...
    int diff = DU[0] - DC;
    if (diff == 0) {
        jo_writeBits(out, bitBuf, bitCnt, HTDC[0]);
    } else {
        unsigned short bits[2];
        jo_calcBits(diff, bits);
        jo_writeBits(out, bitBuf, bitCnt, HTDC[bits[1]]);
        jo_writeBits(out, bitBuf, bitCnt, bits);
    }
    // Encode ACs
    int end0pos = 63;
    ...
    if(end0pos == 0) {
        jo_writeBits(out, bitBuf, bitCnt, EOB);
        return DU[0];
    }
    for(int i = 1; i <= end0pos; ++i) {
        ...
        if ( nrzeroes >= 16 ) {
            ...
            for (int nrmarker=1; nrmarker <= lng; ++nrmarker)
                jo_writeBits(out, bitBuf, bitCnt, M16zeroes);
            nrzeroes &= 15;
        }
        unsigned short bits[2];
        jo_calcBits(DU[i], bits);
        jo_writeBits(out, bitBuf, bitCnt, HTAC[(nrzeroes<<4)+bits[1]]);
        jo_writeBits(out, bitBuf, bitCnt, bits);
    }
    if(end0pos != 63) {
        jo_writeBits(out, bitBuf, bitCnt, EOB);
    ...
    return DU[0];
        ...
        //const void *data, int width, int height, int comp, int quality
...
public:
    ...
    variant process_(const variant& data_in) override {
        ...
        out.writeBytes(sizeof(head2), head2);
        ...
        for(int x = 0; x < width; x += 8) {
            ...
            for(int row = y, pos = 0; row < y+8; ++row) {
                for(int col = x; col < x+8; ++col, ++pos) {
                    ...
                    float r = imageData[p+ofsR], g = imageData[p+ofsG], b = imageData[p+ofsB];
            ...
            DCY = jo_processDU(out, bitBuf, bitCnt, YDU, fdtbl_Y, DCY, YDC_HT, YAC_HT);
            DCU = jo_processDU(out, bitBuf, bitCnt, UDU, fdtbl_UV, DCU, UVDC_HT, UVAC_HT);
            DCV = jo_processDU(out, bitBuf, bitCnt, VDU, fdtbl_UV, DCV, UVDC_HT, UVAC_HT);
        }
    }
